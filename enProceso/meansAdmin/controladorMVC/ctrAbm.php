<?php
/*********************************************************************
* Codigo del archivo	: PHP_021_ctrAbm
* Nombre del archivo	: ctrAbm.php
...
class ctrAbm extends clControlador{
	...
	public function mPost($pAction, $pTableName,  $pPrincipalID = ''){
		...
		$this->entAbm->arrPost			= $_POST;
		
		if($pAction == 'create'){
			$this->clModelo->mExecuteInsert($this->entAbm->arrPost, $this->entAbm->vTableName);
		}elseif($pAction == 'edit'){
			$this->clModelo->mExecuteUpdate($this->entAbm->arrPost, $this->entAbm->vTableName, $this->entAbm->vCond);
		}
		
		header('Location: '.C_PREFIX_HOME.'/abm/show/'.$pTableName);
	}
